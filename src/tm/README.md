...
### [使用工厂类创建线程](tm_08/Main.java "查看示例")

使用工厂类，可以将对象的创建集中化，有以下几个好处：

- 更容易修改类，或者改变创建线程的方式。

- 更容易为有限资源限制创建对象的数目。例如，可以限制一个类型的对象不多于 n 个。

- 更容易为创建的对象生成统计信息。

Java 提供了 ThreadFactory 接口，这个接口用以实现线程工厂。一些 Java 同步 API 的高级工具就使用线程工厂类来创建对象。 